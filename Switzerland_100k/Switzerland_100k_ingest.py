...
if __name__ == '__main__':
    ...
    logFile = os.path.join(log_directory,'/big2/dc/ingest/Switzerland_100k' + time.strftime('%y_%m_%d') + '.log')
    ...
    mods_directory = os.path.join(source_directory, '/big2/dc/ingest/Switzerland_100k/mods')
    if not os.path.isdir(mods_directory):
        logging.error('MODS directory invalid \n')
        sys.exit()
    
    tif_directory = os.path.join(source_directory, '/big2/dc/ingest/Switzerland_100k/tif')
    if not os.path.isdir(tif_directory):
        logging.error('TIF directory invalid \n')
        sys.exit()

    jpg_med_directory = os.path.join(source_directory, '/big2/dc/ingest/Switzerland_100k/jpg_medium')
    if not os.path.isdir(jpg_med_directory):
        logging.error('JPG medium directory invalid \n')
        sys.exit()

    jpg_thumb_directory = os.path.join(source_directory, '/big2/dc/ingest/Switzerland_100k/jpg_thumb')
    if not os.path.isdir(jpg_thumb_directory):
        logging.error('JPG thumbnail directory invalid \n')
        sys.exit()    

    jp2_lossy_directory = os.path.join(source_directory, '/big2/dc/ingest/Switzerland_100k/jp2_lossy')
    if not os.path.isdir(jp2_lossy_directory):
        logging.error('JP2 lossy directory invalid \n')
        sys.exit()
    
    jp2_lossless_directory = os.path.join(source_directory, '/big2/dc/ingest/Switzerland_100k/jp2_lossless')
    if not os.path.isdir(jp2_lossless_directory):
        logging.error('JP2 lossless directory invalid \n')
        sys.exit()
   
    fits_directory = os.path.join(source_directory, '/big2/dc/ingest/Switzerland_100k/fits')
    ...
    do ingest
    '''
    #put in Switzerland_100k object
    try:
        collection_label = u'8889'
